...
def main(args):
    ...
    while cap.isOpened():
        ...
        if len(trackers) > 5:
            ...
            print(
                "dx:",
                round(trackers[0].velocity.speed[0].speed, 2),
                "dy:",
                round(trackers[0].velocity.speed[1].speed, 2),
                "angle:",
                round(trackers[0].velocity.direction * 180 / 3.1416, 2),
            )
        ...
        if args.draw_detection:
            ...
            detections_frame = drawutils.draw_detections(
                detections_frame, detection_bbs
            )
            cv.rectangle(
                detections_frame,
                (
                    detection_roi[0],
                    detection_roi[1],
                    detection_roi[2] - detection_roi[0],
                    detection_roi[3] - detection_roi[1],
                ),
                (255, 0, 0),
                2,
            )
        ...
        if args.draw_tracking:
            ...
            tracking_frame = drawutils.draw_detections(
                tracking_frame, new_detections, (0, 0, 255)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", type=str, help="Location for the video sequence", required=True
    )
    parser.add_argument(
        "--draw_detection", type=bool, help="Enable the detection bbs", default=False
    )
    parser.add_argument(
        "--draw_tracking", type=bool, help="Enable the tracking bbs", default=False
    )
    parser.add_argument(
        "--sample_tracking",
        type=int,
